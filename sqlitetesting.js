...
ds18x20.get("28-0316c2c8bbff", function(err, value) {
  ...
  db.each(`SELECT temperatureid, date, time, location, temperature FROM langs WHERE date = '2018_05_16'`, (err, row) => {
    if (err){
      throw err;
    }
    console.log(`${JSON.stringify(row)}`);
...
ds18x20.get("28-0316c2c8bbff", function(err, value) {
  ...
  db.each(`SELECT temperatureid, date, time, location, temperature FROM langs WHERE ${date.split('_')[2]} = '16' BETWEEN `, (err, row) => {
    if (err){
      throw err;
    }
    console.log(`${JSON.stringify(row)}`);
